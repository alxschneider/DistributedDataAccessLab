...
public class ProductsController : ControllerBase
{
        ...
        [FromQuery] string? search)
    {
        ...
        var result = products.Select(p => new ProductResponseDto(
            p.Id, p.SellerId, p.Name, p.Description, p.Category,
            p.Price, p.Stock, p.DiscountPercent, p.FinalPrice,
    ...
    public async Task<IActionResult> GetById(int id)
    {
        ...
        return Ok(new ProductResponseDto(
            ...
            product.Category, product.Price, product.Stock, product.DiscountPercent,
            product.FinalPrice, product.ImageUrl, product.CreatedAt, product.UpdatedAt
        ));
    }
    ...
    public async Task<IActionResult> Create([FromBody] CreateProductDto dto)
    {
        ...
        var sellerExists = await _customerClient.SellerExistsAsync(dto.SellerId);
        if (!sellerExists)
    ...
    /// </summary>
    [HttpPatch("{id}/stock")]
    public async Task<IActionResult> UpdateStock(int id, [FromBody] StockUpdateDto request)
    {
        var product = await _context.Products.FindAsync(id);
        if (product == null || !product.IsActive)
            return NotFound();
        ...
        return Ok(new ProductStockDto(product.Id, product.Stock));
    }
    ...
    public async Task<IActionResult> UpdateDiscount(int id, [FromBody] DiscountUpdateDto request)
    {
        var product = await _context.Products.FindAsync(id);
        if (product == null || !product.IsActive)
            return NotFound();
        ...
        return Ok(new ProductDiscountDto(product.Id, product.DiscountPercent, product.FinalPrice));
    }
    ...
    public async Task<IActionResult> DecrementStock(int id, [FromBody] StockDecrementDto request)
    {
        var product = await _context.Products.FindAsync(id);
        if (product == null || !product.IsActive)
            return NotFound();
    ...
    public async Task<IActionResult> IncrementStock(int id, [FromBody] StockDecrementDto request)
    {
        var product = await _context.Products.FindAsync(id);
        if (product == null || !product.IsActive)
            return NotFound();
    ...
    public async Task<IActionResult> GetBySeller(int sellerId)
    {
        ...
        return Ok(products.Select(p => new SellerProductDto(
            p.Id, p.Name, p.Category, p.Price, p.Stock,
            p.DiscountPercent, p.FinalPrice, p.CreatedAt
        )));
    }
    ...
    public async Task<IActionResult> Dashboard()
    {
        ...
        var byCategory = products
            ...
            .Select(g => new CategoryStatsDto(g.Key, g.Count(), Math.Round(g.Average(p => p.Price), 2)))
            .OrderByDescending(x => x.Count)
            .ToList();

        return Ok(new ProductDashboardDto(
            totalProducts, averagePrice, averageStock, outOfStock, withDiscount, byCategory
        ));
    }
    ...
    public async Task<IActionResult> SellerDashboard(int sellerId)
    {
        ...
        return Ok(new SellerDashboardDto(
            products.Count,
            products.Count > 0 ? Math.Round(products.Average(p => p.Price), 2) : 0,
            products.Sum(p => p.Stock),
            products.Count(p => p.Stock == 0),
            products.Count(p => p.DiscountPercent > 0),
            Math.Round(products.Sum(p => p.FinalPrice * p.Stock), 2)
        ));
    }
}
